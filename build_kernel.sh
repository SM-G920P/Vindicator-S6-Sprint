...
BOOTIMG=$BUILD_KERNEL_DIR/build_kernel/boot.img
BOOTIMG_DIR=build_kernel/boot
BOOTIMG_DIR_2=build_kernel
...
OUTPUT_DIR=$BUILD_KERNEL_DIR/build_kernel/output_kernel
ZIP_VER=`sed -n '8p' vindicator`
DEVICE_VER=`sed -n '4p' vindicator`
VER=`sed -n '6p' vindicator`
SU_VER=`sed -n '14p' vindicator`
KERNEL_NAME="$ZIP_VER$DEVICE_VER$VER"
TOOLCHAIN=`sed -n '12p' vindicator`
BUILD_CROSS_COMPILE=$TOOLCHAIN_DIR/$TOOLCHAIN


#vars
KERNEL_DEFCONFIG=custom_defconfig
BASE_VER=`sed -n '2p' vindicator`
DEVICE_VER=`sed -n '4p' vindicator`
VER=`sed -n '6p' vindicator`
...
BUILD_KERNEL()
{
	...
	export KBUILD_BUILD_USER=vindicator
