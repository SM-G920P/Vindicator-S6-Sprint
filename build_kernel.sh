...
RAMFS_TMP="/tmp/vindicator-kernel-exynos7420-ramdisk"
...
if [ -e boot.img ] ; then
	rm vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat version)".zip 2>/dev/null
	...
	cd kernelzip/
	7z a -mx9 vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat ../version)"-tmp.zip *
	zipalign -v 4 vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat ../version)"-tmp.zip ../vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat ../version)".zip
	rm vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat ../version)"-tmp.zip
	cd ..
	ls -al vindicator-kernel-"$(git rev-parse --abbrev-ref HEAD)"-"$(cat version)".zip
